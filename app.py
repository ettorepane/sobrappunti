...
@app.route('/view/<year>/<subject>')
def view_notes(year, subject):
    path = os.path.join(app.config['UPLOAD_FOLDER'], year, subject)
    if os.path.exists(path):
        files = os.listdir(path)
    else:
        files = []
    return render_template('view.html', year=year, subject=subject, files=files)
...
def upload():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            save_path = os.path.join(app.config['UPLOAD_FOLDER'], year, subject)
            os.makedirs(save_path, exist_ok=True)
            file.save(os.path.join(save_path, filename))
            return redirect(url_for('view_notes', year=year, subject=subject))
